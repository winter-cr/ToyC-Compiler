...
### Issue 19：f16_complex_syntax 编译器异常

| 项目 | 内容 |
|------|------|
| **现象** | 四轮皆为"编译器异常"，编译器返回非零退出码 |
| **可能原因** | ① Parser 递归下降对深度嵌套的语法结构栈溢出；② 语义分析对复杂 AST 组合有未覆盖的检查逻辑导致误报错；③ AstToIr 对某种 AST 节点组合产生无效 IR，被 validate 拒绝 |
| **应由谁修复** | **A**（若为 Parser 问题）或 **B**（若为语义分析误报）或 **C**（若为 AstToIr 缺陷）。优先级：B > C > A |
| **为何未修** | 无 OJ 测试用例，无法复现和定位。需要 OJ 提供 f16 的 ToyC 源码才能精准修复 |

### Issue 20：f18_many_variables 编译器异常

| 项目 | 内容 |
|------|------|
| **现象** | 四轮皆为"编译器异常" |
| **可能原因** | ① 大量局部变量导致栈帧过大，slot 偏移超出限制；② 符号表对大量符号的处理有性能或正确性问题；③ AstToIr 的 `createLocal` 对大量连续创建有边界 bug |
| **应由谁修复** | **C**（若为代码生成栈帧问题）或 **B**（若为符号表问题）。优先级：C > B |
| **为何未修** | 同上，无测试用例无法复现 |
...
| 四 | 93.33 | 6.82 | 71.70 | 全局lookup O(1) |

**功能分提升轨迹：66.67 → 76.67 → 93.33 → 93.33（+26.66，接近满分）**
...
| 成员 | Issues | 数量 | 说明 |
|:---:|---|:---:|------|
| **A** | #19（可能） | 0-1 | 若 f16 为 Parser 深度递归栈溢出 |
| **B** | #1-4, #16, #19（可能）, #20（可能） | 5-7 | AST 适配缺失 + void 语义检查 + f16/f18 若为语义问题 |
| **C** | #5, #12-15, #17-18, #19（可能）, #20（可能） | 7-9 | AST→IR 缺失 + 寄存器策略 + 作用域/全局/顺序 + 性能优化不足 |
...
| 评分项 | 得分 | 满分 | 说明 |
|------|:---:|:---:|------|
| 功能测试 | 93.33 | 100 | 28/30 通过，f16/f18 需测试用例才能修复 |
| 性能测试 | 6.82 | 100 | 4 超时 + 8 通过但效率低（需更优寄存器分配） |
| 实践报告 | — | 20% | 架构/测试/CI/数据文档齐全 |
| **总分** | **71.70** | 100 | — |

### 已知限制

1. 寄存器分配采用全栈方案（无 callee-saved 寄存器复用），每条指令都有额外 load/store 开销
2. 循环无优化（无循环展开、无强度削减），导致 p07-p10 超时
3. f16/f18 因无 OJ 测试用例无法定位根因

> 详细数据见 `docs/第一次测试数据收集.md`、`docs/第二次测试数据收集.md`、`docs/第三次测试数据收集.md`
